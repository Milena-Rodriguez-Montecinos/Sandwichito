...
describe('User POST request negative test', () => {
    ...
    afterAll(() => {
        return HttpRequestManager.makeRequest('DELETE', userByIdURI.replace("{id}", id), '', 'user-four')
    ...
    test.each([
        ...
        return HttpRequestManager.makeRequest('POST', userURI, payloadList.Invalid.POST[key], 'user-four')
        .then(function (response) {
